...
samples.to.be.excluded <- c('SS023.BAM', 'P8C1.BAM', 'P2C1.BAM')
...
CorLevelPlot(heatmap.data,
             x = names(heatmap.data)[43:44],  # Muestra los fenotipos (Healthy y Cancer)
             y = names(heatmap.data)[1:42],  # Muestra todos los módulos (del 1 al 22)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
gene.signf.corr.Healthy <- cor(data_wgcna_t, traits$Healthy, use = 'p')
colnames(gene.signf.corr.Healthy) <- c('Per_cor')
gene.signf.corr.pvals.Healthy <- corPvalueStudent(gene.signf.corr.Healthy, nSamples)
colnames(gene.signf.corr.pvals.Healthy) <- c('pvals')
gene.signf.Healthy <- cbind(gene.signf.corr.Healthy, gene.signf.corr.pvals.Healthy)
gene.signf.Healthy <- data.frame(gene.signf.Healthy)
write.csv(gene.signf.Healthy, "./PBMCs_Assoc_genes_total.csv" , row.names = TRUE)

setDT(gene.signf.Healthy, keep.rownames = "ENSEMBL_ID")  
gene.signf.Healthy_anno <- merge(gene.signf.Healthy,temp_ann,by.x= "ENSEMBL_ID", by.y="ensembl_gene_id_version")
write.csv(gene.signf.Healthy_anno, "./Healthy_Assoc_genes_anno.csv" , row.names = TRUE)

# Para Paciente:
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
gene.signf.corr.Cancer <- cor(data_wgcna_t, traits$Cancer, use = 'p')
colnames(gene.signf.corr.Cancer) <- c('Per_cor')
gene.signf.corr.pvals.Cancer <- corPvalueStudent(gene.signf.corr.Cancer, nSamples)
colnames(gene.signf.corr.pvals.Cancer) <- c('pvals')
gene.signf.Cancer <- cbind(gene.signf.corr.Cancer, gene.signf.corr.pvals.Cancer)
gene.signf.Cancer <- data.frame(gene.signf.Cancer)
write.csv(gene.signf.Cancer, "./Cancer_Assoc_genes_total.csv", row.names = TRUE)

setDT(gene.signf.Cancer, keep.rownames = "ENSEMBL_ID")  
gene.signf.Cancer_anno <- merge(gene.signf.Cancer, temp_ann, by.x= "ENSEMBL_ID", by.y="ensembl_gene_id_version")
write.csv(gene.signf.Cancer_anno, "./Cancer_Assoc_genes_anno.csv" , row.names = TRUE)

# ------------------------------------------------------------------------------
# Seleccionar modulos de interes y graficar su expresion
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
modules_of_interest = c("greenyellow",
                        "red")
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#setwd("pbmcs/Resultados/WGCNA")
save.image(file="./wgcna_work_data.RData")
load(file="./wgcna_work_data.RData") # <- para cargar el trabajo
setwd("pbmcs/Resultados/WGCNA")
# =====================================================================================
# *******************************  Cytoscape   ****************************************
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
